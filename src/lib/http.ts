...
export async function getJson<T>(url: string): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`GET请求失败: ${url} - HTTP ${response.status} ${response.statusText} - ${errorText}`)
  }
...
export async function postJson<T>(url: string, body: any): Promise<T> {
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => '')
    throw new Error(`POST请求失败: ${url} - HTTP ${response.status} ${response.statusText} - ${errorText}`)
  }
