...
function getUploadConfig() {
  ...
  if (!accessKeyId || !secretAccessKey || !bucket || !endpoint) {
    throw new Error('缺少必要的上传配置环境变量：UPLOAD_ACCESS_KEY_ID, UPLOAD_SECRET_ACCESS_KEY, UPLOAD_BUCKET, UPLOAD_ENDPOINT')
  }
...
export async function uploadBufferToS3Like(buf: Buffer, contentType: string): Promise<string> {
  ...
  let secretAccessKey = decodeSecretKey(config.secretAccessKey)
  ...
  const opts: any = {
    ...
    headers: {
      ...
      'x-amz-acl': 'public-read', // 设置文件为公共可读
